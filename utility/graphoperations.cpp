...
void GraphOperations::bfsTraversal(const Graph *graph, ui root_vertex, TreeNode *&tree, VertexID *&bfs_order) {
...
//    bfs_order = new VertexID[vertex_num];
    ...
    for (int i = 0; i < vertex_num; ++i) {
        ui u = bfs_order[i];
        if (tree[u].children_count_) {
            for (int j = 0; j < tree[u].children_count_; ++j) {
                ui u_nbr = tree[u].children_[j];
                ui count;
                auto l = graph->getVertexDNeighbors(u, count);
                auto idx = std::lower_bound(l, l + count, u_nbr);
                bool ok = false;
                if (*idx == u_nbr && idx < l + count && count != 0) {
                    tree[u].child_direct_[u_nbr] = 1;
    //                    tree[u_nbr].child_direct_[u] = -1;
                    ok = true;
                } else {
                    tree[u].child_direct_[u_nbr] = -1;
    //                    tree[u_nbr].child_direct_[u] = 1;
                ...
                if (ok) {
                    l = graph->getVertexDNeighbors(u_nbr, count);
                    idx = std::lower_bound(l, l + count, u);
                    if (*idx == u && idx < l + count) {
                        tree[u].child_direct_[u_nbr] = 0;
                        tree[u_nbr].child_direct_[u] = 0;
                    }
                }
            }
        }
