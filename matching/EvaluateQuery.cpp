    ...
    while (true) {
        while (idx[cur_depth] < idx_count[cur_depth]) {
            ...
            if (cur_depth == max_depth - 1) {
                ...
                cur_depth += 1;
                idx[cur_depth] = 0;
//                generateValidCandidates(data_graph, cur_depth, embedding, idx_count, valid_candidate, visited_vertices, bn, bn_count, order, candidates, candidates_count);
            }
        }
...
void EvaluateQuery::generateValidCandidates(const Graph *data_graph, ui depth, ui *embedding, ui *idx_count,
                                            vector<vector<ui>>& valid_candidate, bool *visited_vertices, ui **bn, ui *bn_cnt,
...
EvaluateQuery::exploreCECIStyle(const Graph *data_graph, const Graph *query_graph, TreeNode *tree, ui **candidates,
                                ...
                                ui *order, size_t &output_limit_num, size_t &call_count, std::vector<vector<int>>& cur_match_res,
                                ui *idx,ui *idx_count,ui *embedding,vector<vector<ui>>& valid_candidates,bool *visited_vertices) {
//    auto begin = std::chrono::high_resolution_clock::now();
    ...
    double memoty_time = 0;
    double cal_time = 0;
//    auto memory_time_1 = std::chrono::high_resolution_clock::now();
    // Allocate the memory buffer.
//    ui *idx = new ui[max_depth];
//    ui *idx_count = new ui[max_depth];
//    ui *embedding = new ui[max_depth];
    vector<ui> temp_buffer(max_valid_candidates_count);
...
//    auto memory_time_2 = std::chrono::high_resolution_clock::now();
//    memoty_time += std::chrono::duration_cast<std::chrono::nanoseconds>(memory_time_2 - memory_time_1).count();
    ...
    // valid_candidates[cur_depth] all start_vertex candidates
    for (ui i = 0; i < idx_count[cur_depth]; ++i) {
    ...
    while (true) {
        while (idx[cur_depth] < idx_count[cur_depth]) {
            ...
            if (cur_depth == max_depth - 1) {
                ...
                vec_failing_set[cur_depth - 1] |= vec_failing_set[cur_depth];
#endif
                cur_match_res.emplace_back(embedding, embedding + max_depth);
//                if (cur_match_res.size() % 100 == 0)
//                    cout << cur_match_res.size() << endl;
                if (embedding_cnt >= output_limit_num) {
                    goto EXIT;
                }
            } else {
    ...
    // Release the buffer.
    EXIT:
//    memory_time_1 = std::chrono::high_resolution_clock::now();
//    delete[] idx;
//    delete[] idx_count;
//    delete[] embedding;
//    delete[] visited_vertices;
    for (int i = 0; i < max_depth; ++i) {
        valid_candidates[i].clear();
    }
//    delete[] valid_candidates;
//    memory_time_2 = std::chrono::high_resolution_clock::now();
//    memoty_time += std::chrono::duration_cast<std::chrono::nanoseconds>(memory_time_2 - memory_time_1).count();
//    auto end = std::chrono::high_resolution_clock::now();
//    double each_time = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count();
//    cout << NANOSECTOSEC(each_time) * 1e6 << " " << NANOSECTOSEC(memoty_time) * 1e6 << " " << NANOSECTOSEC(cal_time) * 1e6 << endl;
    return embedding_cnt;
}

void EvaluateQuery::generateValidCandidates(ui depth, ui *embedding, ui *idx_count, vector<vector<ui>>& valid_candidates, ui *order,
                                            vector<ui>& temp_buffer, TreeNode *tree,
                                            std::vector<std::unordered_map<VertexID, std::vector<VertexID>>> &TE_Candidates,
                                            std::vector<std::vector<std::unordered_map<VertexID, std::vector<VertexID>>>> &NTE_Candidates) {
    ...
    for (ui i = 0; i < tree[u].bn_count_; ++i) {
        ...
        ComputeSetIntersection::ComputeCandidates(current_candidates, current_candidates_count,
                                                  valid_candidates[depth].data(), valid_candidates_count,
                                                  temp_buffer.data(), temp_count);
