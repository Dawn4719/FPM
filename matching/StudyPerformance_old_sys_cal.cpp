...
//    printf("Avg queue size: %llu\n", sizeof(int) * (avg_res + avg_res2) / calculate_count);
    fin << "base" << "," << PATH << "," << QUE << ","<< QUERY_NUMS << "," << D << ","
        << cur_sp_match_res << ","
        << NANOSECTOSEC(MatchTime) << ","
        << NC(load_graphs_time_in_ns) << ","
        << NC(MatchTime) << ","
        << try_count << ","
        << get_index_mem()["pk"] - memory[0] << ","
        << memory[1] - memory[0] << ","
        << NC(MatchTime - time4) << ","
        << NC(time4) << ",";
    for (int j = 0; j < QUERY_NUMS; ++j) {
        cout << "q " << j << " " << query_graph[j]->emb_count << endl;
        fin << query_graph[j]->emb_count << ",";
    }
    fin << endl;
...
//        cur_sp_match_res = 0;
    ...
    printf("ALL Time: %.4lf\n", NANOSECTOSEC(MatchTime));
//    printf("Avg queue size: %llu\n", sizeof(int) * (avg_res + avg_res2) / calculate_count);
    fin << "BFS_base" << "," << PATH << "," << QUE << ","<< QUERY_NUMS << "," << D << ","
        << cur_sp_match_res << ","
        << NC(MatchTime) << ","
        << NC(total_time_in_ns) << ","
        << NC(totime) << ","
