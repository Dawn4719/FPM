...
FilterVertices::CECIFilter(const Graph *data_graph, const Graph *query_graph, ui **&candidates, ui *&candidates_count,
                           ...
                           int start, int data_start_vertex, bool* st, int SPECIAL) {
//    std::cout << 1 << std::endl;
    GenerateFilteringPlan::generateCECIFilterPlan(data_graph, query_graph, tree, order, start);
//    std::cout << 2 << std::endl;
    allocateBuffer(data_graph, query_graph, candidates, candidates_count, start);
    ...
    computeCandidateWithNLF(data_graph, query_graph, root, candidates_count[root], candidates[root], data_start_vertex);
//    auto t1e = gti;
//    time[0] += cti(t1e, t1b);
//    std::cout << 3 << std::endl;
...
//    auto t2b = gti;
    std::unordered_map<ui, ui> flag;
    std::unordered_map<ui, ui> updated_flag;
//    memset(flag, 0, sizeof(ui) * data_graph->vertices_count_);
//    memset(updated_flag, 0, sizeof(ui) * data_graph->vertices_count_);

//    std::fill(flag.begin(), flag.end(), 0);
    std::vector<bool> visited_query_vertex(query_vertices_count, false);
...
//    std::cout << 4 << std::endl;
    for (ui i = 1; i < query_vertices_count; ++i) {
        VertexID u = order[i];
        TreeNode& u_node = tree[u]; // current query tree node
        VertexID u_p = tree[u].parent_; // current query tree node's father

        short query_direction;
        if (data_graph->has_dir_) query_direction = tree[u_p].child_direct_[u];

        ui u_label = query_graph->getVertexLabel(u);
        ui u_degree = query_graph->getVertexDegree(u);
        ui u_ddegree = query_graph->getVertexDDegree(u);
        ...
        VertexID* frontiers = candidates[u_p]; // u_p's candidate nodes list
        ui frontiers_count = candidates_count[u_p]; // u_p's candidate nodes counts

        // iterate u_p's candidate nodes
        for (ui j = 0; j < frontiers_count; ++j) {
            VertexID v_f = frontiers[j]; // get current u_p's candidate node v_f

            if (v_f == INVALID_VERTEX_ID)
                continue;

//            if (st != nullptr && v_f != data_start_vertex && st[v_f]) { continue;}
            ui nbrs_cnt;
            const VertexID* nbrs = data_graph->getVertexNeighbors(v_f, nbrs_cnt); // v_f's neighbors
            auto iter_pair = TE_Candidates[u].emplace(v_f, std::vector<VertexID>());
            // iterate v_f's neighbors
            for (int k = 0; k < nbrs_cnt; k += SPECIAL) {
                VertexID v = nbrs[k]; // get current v_f's neighbor v
//                if ((st != nullptr && v != data_start_vertex && !st[v]) || st == nullptr) {
                    if (data_graph->getVertexLabel(v) == u_label && data_graph->getVertexDDegree(v) >= u_ddegree &&
                            data_graph->getVertexDegree(v) - data_graph->getVertexDDegree(v) >= u_degree - u_ddegree) {
                        // direction check
                        if (data_graph->has_dir_) {
                            ui count;
                            int direction;
                            auto l = data_graph->getVertexDNeighbors(v_f, count);
                            auto idx = std::lower_bound(l, l + count, v);
                            if (*idx == v && idx < l + count && count != 0) {
                                direction = 1;
                            } else {
                                direction = -1;
                            }

                            l = data_graph->getVertexDNeighbors(v, count);
                            idx = std::lower_bound(l, l + count, v_f);
                            if (*idx == v_f && idx < l + count && count != 0 && direction == 1) {
                                direction = 0;
                            }
                            if (direction != 0 && direction != query_direction)
                                continue;
                        }
                        ...
                        if (v_nlf->size() >= u_nlf->size()) {
                            ...
                            if (is_valid) {
//                                if (unique_st.find(v) == unique_st.end()) {
//                                    if ((st != nullptr && v != data_start_vertex && !st[v]) || st == nullptr)
...
//                }
        ...
        for (ui k = 0; k < candidates_count[u]; ++k) {
            VertexID v = candidates[u][k];
            flag[v] = 0;
        }
    }
//    std::cout << 5 << std::endl;
...
//            assert(tree[u_p].child_direct_.find(u) != tree[u_p].child_direct_.end());
            int query_direction;
            if (data_graph->has_dir_) query_direction = tree[u_p].nte_child_direct_[u];
...
//                if (st != nullptr && v_f != data_start_vertex && st[v_f]) { continue;}
                ...
                for (ui k = 0; k < nbrs_cnt; k += SPECIAL) {
                    VertexID v = nbrs[k];
//                    if ((st != nullptr && v != data_start_vertex && !st[v]) || st == nullptr) {
                        if (data_graph->getVertexLabel(v) == u_label && data_graph->getVertexDDegree(v) >= u_ddegree &&
                                data_graph->getVertexDegree(v) - data_graph->getVertexDDegree(v) >= u_degree - u_ddegree) {
                            ...
                            if (data_graph->has_dir_) {
                                ui count;
                                int direction;
                                auto l_ = data_graph->getVertexDNeighbors(v_f, count);
                                auto idx = std::lower_bound(l_, l_ + count, v);
                                if (*idx == v && idx < l_ + count && count != 0) {
                                    direction = 1;
                                } else {
                                    direction = -1;
                                }

                                l_ = data_graph->getVertexDNeighbors(v, count);
                                idx = std::lower_bound(l_, l_ + count, v_f);
                                if (*idx == v_f && idx < l_ + count && count != 0 && direction == 1) {
                                    direction = 0;
                                }
                                if (direction != 0 && direction != query_direction)
                                    continue;
...
//    std::cout << 6 << std::endl;
    std::unordered_map<ui,ui> local_cardinality;
    // Reverse BFS refine.
...
//    std::vector<ui> local_cardinality(data_vertices_count, 0);
//    std::fill(local_cardinality.begin(), local_cardinality.end(), 0);
    ...
    for (int i = query_vertices_count - 1; i >= 0; --i) {
        ...
        for (ui j = 0; j < u_node.bn_count_; ++j) {
            ...
            flag_num += 1;
            for (auto & iter : NTE_Candidates[u][u_bn]) {
...
FilterVertices::computeCandidateWithNLF(const Graph *data_graph, const Graph *query_graph, VertexID query_vertex,
                                               ui &count, ui *buffer, int data_start) {
    LabelID label = query_graph->getVertexLabel(query_vertex);
    ui ddegree = query_graph->getVertexDDegree(query_vertex);
...
//        }
    ...
    for (ui j = 0; j < data_vertex_num; ++j) {
        ui data_vertex = data_vertices[j];
        if (data_graph->getVertexDDegree(data_vertex) >= ddegree
            && data_graph->getVertexDegree(data_vertex) - data_graph->getVertexDDegree(data_vertex) >= degree - ddegree) {
