...
// vector<map<int, vector<int>>> get_nei_label_vertex;
int top = 0x3f3f3f3f;
int mx = -1;
struct eg{
    int a;
    ...
    int d;
    eg()=default;
    eg(int _a, int _b, int _c, int _d) {
        a = _a;
        b = _b;
        c = _c;
        d = _d;
...
// map<int, map<int, int>> eg2id;
// vector<vector<int>> eg2id;
map<int, int> lab;
...
// map<int, map<int, bool>> Nst;
map<int, vector<map<int, map<int, bool>>>> Nst;
map<int, map<int, bool>> n_st;
//fstream fss = fstream("../../graph_path.txt", ios::app);
...
//        mtx.unlock();
//        for (auto i : now) {
//            for (auto j : N[i.first].n[i.second].match) {
//                cout << j << " ";
//            }
//            cout << " ";
//        }
...
//        auto timmm = std::chrono::duration_cast<std::chrono::nanoseconds>(eeend - sstart).count();
        if (cur_sp_match_res % 1000 == 0) {
            cout << cur_sp_match_res << " " << (get_index_mem()["pk"] - memory[0]) * 1.0 / 1024 / 1024 << endl;
            cout << "nei_all_size " << (double)max_size * 4 / 1024 << "kB" << endl;
...
void begin_extend(Graph* data_graph, Graph** query_graph) {
...
//        cout<< 'bb' << endl;
    ...
    for (auto i : match_result[0]) sum += i;
    N[0].n.resize(1);
    N[0].n[0].match = match_result[0];
    N[0].n[0].step = 0;
...
//    if (new_la > la2) {
//        la2 = new_la;
        size_t all_cnt = tree.size() * 4;
        all_cnt += 3 * sizeof(void*) + sizeof(char);
        for (const auto& i : tree) {
            all_cnt += i.second.size() * 4 * 2;
            for (const auto& j : i.second) {
                all_cnt += 3 * sizeof(void*) + sizeof(char);
                for (const auto& l : j) {
                    all_cnt += 4;
                    all_cnt += l.second.size() * 4;
                }
            }
        }
        for (auto & i : N) {
...
//            }
        ...
        max_cal_size = max(max_cal_size, all_cnt);
//    }
...
void bfs_init_solve(double total_time_in_ns, vector<vector<int>>& all_match_res, Graph** query_graph, Graph* data_graph) {
    ...
    cout << "PATH: " << PATH << " Query_Num: " << QUERY_NUMS << " D: " << D << " Meth: bfs_init" << " top: " << top << dl;
...
//    get_nei_label_vertex.resize(data_graph->vertices_count_);
    ...
    for (auto & all_match_re : all_match_res) {
        match_result.clear();
        match_result.emplace_back(all_match_re);
        auto each_begin = std::chrono::high_resolution_clock::now();
        topk = false;
        begin_extend(data_graph, query_graph);
...
//            la = new_la;
//        }
//        if (cur_sp_match_res)break;
//        if (i % (all_match_res.size() / 20) == 0)
//        cout << "each " << i << " " << cur_sp_match_res << " " << NC(each_time) << "ms " << max_cal_size / 1024 << " " << nei_all_size * 4 / 1024  << endl;
        i++;
...
//                la = new_la;
        //            get_nei_label_vertex.resize(data_graph->vertices_count_);
            nei_all_size = get_nei_label_vertex.size();
            for (const auto& iii : get_nei_label_vertex){
    //            fill(get_nei_label_vertex.begin(), get_nei_label_vertex.end(), NULL);
            ...
            nei_all_size *= 4;
            nei_all_size += 3 * sizeof(void*) + sizeof(char);
            max_size = max(max_size, nei_all_size);
    //                cout << nei_all_size * 4 / 1024 << "kB" << endl;
    //            }
            get_nei_label_vertex.clear();
//        }
        if (topk) break;
    ...
    fin << "new" << "," << PATH << "," << QUE << ","<< QUERY_NUMS << "," << D << ","
        ...
        << try_count << ","
        << NC(tim1) << ","
        << NC(tim2) << "," << (get_index_mem()["pk"] - memory[0]) << ","
        << (memory[1] - memory[0]) << ","
