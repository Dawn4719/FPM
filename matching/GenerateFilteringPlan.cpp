...
void GenerateFilteringPlan::generateCECIFilterPlan(const Graph *data_graph, const Graph *query_graph, TreeNode *&tree,
    ...
    if (start == -1)
        start_vertex = selectCECIStartVertex(data_graph, query_graph);
...
//    std::cout << std::endl;
    ...
    if (data_graph->has_dir_) {
        for (ui i = 1; i < query_vertices_num; ++i) {
            ...
            for (ui j = 0; j < u_nbrs_count; ++j) {
                VertexID u_nbr = u_nbrs[j];
                if (u_nbr != tree[u].parent_) {
                    ui count;
                    auto l = query_graph->getVertexDNeighbors(u, count);
                    auto idx = std::lower_bound(l, l + count, u_nbr);
                    bool is_ok = false;
                    if (*idx == u_nbr && idx < l + count && count != 0) {
                        tree[u].nte_child_direct_[u_nbr] = 1;
//                    tree[u_nbr].nte_child_direct_[u] = -1;
                        is_ok = true;
                    } else {
                        tree[u].nte_child_direct_[u_nbr] = -1;
//                    tree[u_nbr].nte_child_direct_[u] = 1;
                    ...
                    if (is_ok) {
                        l = query_graph->getVertexDNeighbors(u_nbr, count);
                        idx = std::lower_bound(l, l + count, u);
                        if (*idx == u && idx < l + count) {
                            tree[u].nte_child_direct_[u_nbr] = 0;
                            tree[u_nbr].nte_child_direct_[u] = 0;
                        }
                    }
                }
            }
...
VertexID GenerateFilteringPlan::selectCECIStartVertex(const Graph *data_graph, const Graph *query_graph) {
    ...
    for (ui i = 0; i < query_graph->getVerticesCount(); ++i) {
        ui degree = query_graph->getVertexDDegree(i);
        ui count = 0;
