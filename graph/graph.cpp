...
void Graph::loadGraphFromFile(const std::string &file_path) {
    ...
    std::vector<ui> dneighbors_offset;
    if (has_dir_) dneighbors_offset.resize(vertices_count_, 0);
    ...
    while (infile >> type) {
        if (type == 'v') { // Read vertex.
            ...
            infile >> id >> label >> degree >> ddegree;

            labels_[id] = label;
            offsets_[id + 1] = offsets_[id] + degree;
            if (has_dir_) {
                offsets_[id + 1] = offsets_[id] + degree;
                doffsets_[id + 1] = doffsets_[id] + ddegree;
            }
            else {
                offsets_[id + 1] = offsets_[id] + ddegree;
            }

            if (ddegree > max_degree_) {
                max_degree_ = ddegree;
            }
            ...
            // build csr
            if (has_dir_){
                ui doffset = doffsets_[begin] + dneighbors_offset[begin];
                dneighbors_[doffset] = end;
                dneighbors_offset[begin] += 1;
            }
    ...
    l_count = labels_frequency_.size();
...
//    for (ui i = 0; i < vertices_count_; ++i) {
//        std::sort(dneighbors_ + doffsets_[i], dneighbors_ + doffsets_[i + 1]);
//    }
